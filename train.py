...
from transformers import AdamW, BertTokenizer, DebertaTokenizer
from pytorch_pretrained_bert import BertAdam
from .model import BertClassifier, AutoBertClassifier
from sklearn.metrics import f1_score, classification_report, accuracy_score
from torch.utils.data import DataLoader, RandomSampler, TensorDataset, SequentialSampler
from tqdm import tqdm

from sentiment.data_processor import read_train_file, read_test_file

warnings.filterwarnings("ignore")

def tokenize_corpus(text_list: List[str], entities, tokenizer: BertTokenizer) -> List[List[int]]:
    tokenized_data = []
    for text, entity in tqdm(zip(text_list, entities), desc="Tokenizing Corpus", leave=True, total=len(text_list)):
...
def mask_token_ids(token_ids: List[List[int]], padded_size: int):
    input_ids = []
    input_types = []
    input_masks = []

    for token in tqdm(token_ids, desc="Mask Token Ids"):
        types = [0] * (len(token))
        masks = [1] * (len(token))
...
def split_test_dataset(input_ids: List[List[int]], input_types: List[List[int]],
                        input_masks: List[List[int]], batch_size: int) -> (DataLoader, DataLoader):
...
def valid_step(model, device, valid_loader):
    ...
    for batch_idx, (x1, x2, x3, y) in tqdm(enumerate(valid_loader), desc="Valid"):
        x1_g, x2_g, x3_g, y_g = x1.to(device), x2.to(device), x3.to(device), y.to(device)
        with torch.no_grad():
...
def test_and_save_reault(model, device, test_loader, test_entitys, test_ids, result_path):
    ...
    for batch_idx, (x1, x2, x3) in tqdm(enumerate(test_loader), desc="Test and Save"):
...
def main(args):
    global train_entities, test_entities, test_ids
    epochs = args.epochs
    ...
    train_corpus, train_labels, train_entities = read_train_file(file_path=args.train_input_path)
    test_corpus, test_entities, test_ids = read_test_file(file_path=args.test_input_path)
    ...
    tokenized_train_data = tokenize_corpus(train_corpus, train_entities, tokenizer) #文本和实体concat
    tokenized_test_data = tokenize_corpus(test_corpus, test_entities, tokenizer)
    print()
    print("tokenized")
    ...
    if args.checkpoint_path is not None:
        ...
        if args.model_type == "Bert":
            model = BertClassifier(bert_path=pretrain_path, dropout_rate=0.2).to(DEVICE)
        elif args.model_type == "Autobert":
            model = AutoBertClassifier(bert_path=pretrain_path).to(DEVICE)
        else :
    ...
    for epoch in tqdm(range(1, epochs + 1), desc="Epoch training"):
    ...
    test_and_save_reault(model, DEVICE, test_loader, test_entities, test_ids, args.result_path)
